...
;;;###autoload
(defvar aide-mode-initialization-hooks (list 'aide-mode-initialize-for-major-mode)
...
(defvaralias 'aide-mode-finalization-hooks 'aide-mode-hook
  "Finalization hooks, which are run at the end of aide-mode.")

;;;###autoload
(defun aide-mode-initialize-for-major-mode (&optional major-mode-)
  "Run initialization hooks for specified major-mode-, \
use current major-mode if not specified."
  (let ((hooks (gethash (or major-mode- major-mode) aide-mode-initialization-hooks-for-major-modes)))
    (run-hooks 'hooks)))
...
(defun aide-mode-finalize-for-major-mode (&optional major-mode-)
  "Run initialization hooks for specified major-mode-, \
use current major-mode if not specified."
  (let ((hooks (gethash (or major-mode- major-mode) aide-mode-finalization-hooks-for-major-modes)))
    (run-hooks 'hooks)))
...
(define-minor-mode aide-mode
  ...
  (let ((hooks (aide-buffer-get '(hooks))))
    (run-hooks 'hooks)))
